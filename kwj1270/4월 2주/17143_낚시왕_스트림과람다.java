...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		sharks = new CopyOnWriteArrayList<>();
		map = new CopyOnWriteArrayList[n][m];
		catchSharks = new CopyOnWriteArrayList<>();

		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				map[i][j] = new CopyOnWriteArrayList<>();
			}
		}
	...
	private static void removeSharkByMap() {
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				...
				map[i][j].stream().forEach(sharks::remove);

				map[i][j] = new CopyOnWriteArrayList<Shark>();
				map[i][j].add(remain);
			}
		}
	...
	private static void sharkMove() {
		sharks.stream()
			.peek(shark -> map[shark.x][shark.y].remove(shark))
			.peek(shark -> shark.move(n, m))
			.forEach(shark -> map[shark.x][shark.y].add(shark));
	}

	private static void findAndRemove(int m) {
		for (int i = 0; i < n; i++) {
			if (map[i][m].size() == 0) {continue;}
			map[i][m].stream()
				.peek(catchSharks::add)
				.peek(sharks::remove)
				.forEach(map[i][m]::remove);
			return;
		}
	}
